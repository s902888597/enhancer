...
class ResidualAdapter(nn.Module):
    def __init__(self, input_dim: int = 1024, hidden_dim: int = 1024, output_dim: int = 768, dropout: float = 0.1, residual_scale: float = 0.1):
        super().__init__()
        ...
        self.align_head = nn.Linear(input_dim, output_dim)
        self.residual_scale = residual_scale

    def forward(self, x: torch.Tensor):
        delta = self.adapter(x)
        adapted = x + self.residual_scale * delta
        align = F.normalize(self.align_head(adapted), p=2, dim=-1)
        return adapted, align, delta
...
def load_pairs_for_split(split: str, ids: list[str], uni_root: Path, loki_root: Path):
    ...
    for sid in ids:
        ...
        rows.append({"split": split, "case_id": sid, "n_pairs": int(x_uni.shape[0]), "n_missing_ids": int(missing)})
        if x_uni.shape[0] > 0:
            xs_uni.append(x_uni)
...
def train_epoch(model, loader, optim, device: torch.device, temperature: float, residual_penalty: float):
    model.train()
    total = 0.0
    n = 0
    for xb, yb in loader:
        ...
        _, pred, delta = model(xb)
        loss_align = clip_loss(pred, yb, temperature)
        loss_reg = delta.pow(2).mean()
        loss = loss_align + residual_penalty * loss_reg
        loss.backward()
        optim.step()
...
def eval_epoch(model, loader, device: torch.device, temperature: float, residual_penalty: float):
    model.eval()
    total = 0.0
    n = 0
    cosine_sum = 0.0
    ...
    with torch.no_grad():
        for xb, yb in loader:
            ...
            count += xb.shape[0]
            delta_norm_sum += float(delta.pow(2).sum(dim=1).sqrt().sum().item())
    return total / max(n, 1), cosine_sum / max(count, 1), delta_norm_sum / max(count, 1)


def adapt_case_mean(model, uni_case: Path, device: torch.device, batch_size: int) -> np.ndarray:
    x, _ = load_case_features(uni_case)
    outs = []
    with torch.no_grad():
        for i in range(0, x.shape[0], batch_size):
            xb = torch.tensor(x[i:i + batch_size], dtype=torch.float32, device=device)
            adapted, _, _ = model(xb)
            outs.append(adapted.cpu().numpy())
...
def build_mean_cache(model, base_cache: Path, uni_root: Path, out_cache: Path, device: torch.device, batch_size: int) -> dict:
    ...
    for split, (x_name, ids_name) in SKCM_FILES.items():
        ...
        feats = []
        for sid in ids:
            feats.append(adapt_case_mean(model, uni_root / sid, device, batch_size))
...
def main() -> None:
    ...
    p.add_argument("--out-dir", default="/taiga/illinois/vetmed/cb/kwang222/enhancer/MIL/average_embed/skcm_uni_to_loki_residual_adapter")
    p.add_argument("--aligned-cache-out", default="/taiga/illinois/vetmed/cb/kwang222/enhancer/MIL/average_embed/uni_loki_residual_skcm_mean_cache")
    ...
    model = ResidualAdapter(hidden_dim=args.hidden_dim, dropout=args.dropout, residual_scale=args.residual_scale).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_epoch(model, train_loader, optim, device, args.temperature, args.residual_penalty)
        val_loss, val_cos, val_delta = eval_epoch(model, val_loader, device, args.temperature, args.residual_penalty)
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "val_mean_cosine": val_cos, "val_delta_norm": val_delta})
        print(f"Epoch {epoch}: train_loss={train_loss:.4f} val_loss={val_loss:.4f} val_cos={val_cos:.4f} val_delta={val_delta:.4f}", flush=True)
    ...
    final_train_loss, train_cos, train_delta = eval_epoch(model, train_loader, device, args.temperature, args.residual_penalty)
    final_val_loss, val_cos, val_delta = eval_epoch(model, val_loader, device, args.temperature, args.residual_penalty)

    torch.save(model.state_dict(), out_dir / "best_adapter.pt")
        ...
        json.dumps(
            {
                ...
                "train_delta_norm": float(train_delta),
                "validation_delta_norm": float(val_delta),
            },
            indent=2,
        )
    )
    ...
    print(f"Saved residual mean cache to {args.aligned_cache_out}", flush=True)
