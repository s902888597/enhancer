...
IN_CSV = ROOT / "three_cancer_top1000_attention_k300_patch_weights" / "all_patch_attention.csv"
OUT_DIR = ROOT / "three_cancer_top1000_attention_k300_patch_weights"
...
def make_plot(rank_df: pd.DataFrame, sample_df: pd.DataFrame, out_path: Path) -> None:
    ...
    for cancer in cancers:
        tmp = rank_df[(rank_df["cancer"] == cancer) & (rank_df["split"] == split)]
    ...
    fig.suptitle("Top1000 Attention k=300: How Attention Is Distributed Across Patches", fontsize=13, y=0.99)
    fig.tight_layout()
...
def main() -> None:
    ...
    make_plot(rank_df, sample_df, OUT_DIR / "attention_k300_overview.png")
