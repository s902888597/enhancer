#!/usr/bin/env python3
"""
Mean-pool patch embeddings per case, then train EnhancerMLP (with residual blocks)
to predict super-enhancer expression.

Inputs:
- train/val/test CSVs with columns: sample, <enhancer...>
- Patch features: features_patches_attention_top10_npy/<split>/<case>/*.npy

Outputs (under --out-dir):
- best_model_enhancer_mlp.pt
- train/val/test ids
- test_pred_enhancer_mlp.npy, test_true_enhancer_mlp.npy
- per_enhancer_correlation_enhancer_mlp.csv (Pearson r on test)
"""
...
# Ensure enhancer_prediction package is on path for its internal config import
import sys
from pathlib import Path as _Path
_pkg_dir = (_Path(__file__).resolve().parents[2] / "our_code_data" / "enhancer_prediction")
if str(_pkg_dir) not in sys.path:
    sys.path.insert(0, str(_pkg_dir))

from our_code_data.enhancer_prediction.models import EnhancerMLP
...
def main():
    ...
    parser.add_argument("--num-residual", type=int, default=2)
    parser.add_argument("--dropout", type=float, default=0.3)
    parser.add_argument("--use-tanh-out", action="store_true", default=False)
    # Keep output unconstrained by default for z-score/log1p targets that can be negative.
    parser.add_argument("--use-relu-out", action="store_true", default=False)
    ...
    model = EnhancerMLP(
        ...
        use_tanh_out=args.use_tanh_out,
        use_relu_out=args.use_relu_out,
    ).to(device)
    ...
    # Save artifacts
    torch.save(model.state_dict(), out_dir / "best_model_enhancer_mlp.pt")
    np.save(out_dir / "train_ids_enhancer_mlp.npy", np.array(train_ids))
    np.save(out_dir / "val_ids_enhancer_mlp.npy", np.array(val_ids))
    np.save(out_dir / "test_ids_enhancer_mlp.npy", np.array(test_ids))
    np.save(out_dir / "test_pred_enhancer_mlp.npy", test_pred)
    np.save(out_dir / "test_true_enhancer_mlp.npy", test_true)
    corr_df.to_csv(out_dir / "per_enhancer_correlation_enhancer_mlp.csv", index=False)
