...
OUT_DIR = ROOT / "three_cancer_top1000_nopca_attention_compare"
...
def build_rows() -> list[dict]:
    return [
        {
            "Cancer": "BRCA",
            "Mean-MLP (no PCA)": test_mean(ROOT / "brca_top1000_mean_mlp_nopca_seed44" / "summary.csv"),
        ...
        {
            "Cancer": "LUAD",
            "Mean-MLP (no PCA)": test_mean(ROOT / "luad_top1000_mean_mlp_nopca_seed44" / "summary.csv"),
        ...
        {
            "Cancer": "SKCM",
            "Mean-MLP (no PCA)": test_mean(ROOT / "skcm_top1000_mean_mlp_nopca_seed44" / "summary.csv"),
...
def save_png(rows: list[dict]) -> Path:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out_png = OUT_DIR / "three_cancer_top1000_nopca_attention_compare.png"
    ...
    for row in rows:
        cell_text.append(
            [
                row["Cancer"],
                f'{row["Mean-MLP (no PCA)"]:.4f}',
                f'{row["Attention (all patches)"]:.4f}',
                f'{row["Attention (300 patches)"]:.4f}',
            ]
        )

    fig, ax = plt.subplots(figsize=(11.2, 3.1), dpi=220)
