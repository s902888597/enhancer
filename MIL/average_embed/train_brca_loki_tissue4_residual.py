...
DEFAULT_IMAGE_CACHE = Path("/taiga/illinois/vetmed/cb/kwang222/enhancer/MIL/average_embed/loki_brca_mean_cache")
DEFAULT_TISSUE_CACHE = Path("/taiga/illinois/vetmed/cb/kwang222/enhancer/MIL/average_embed/tissue4_brca_mean_cache")
LABEL_CSV = {
...
class TissueResidualRegressor(nn.Module):
    def __init__(self, img_dim: int, tissue_dim: int, output_dim: int, hidden_dim: int, cond_dim: int, dropout: float):
        ...
        # Start close to base-only behavior; residual learns corrections.
        nn.init.zeros_(self.residual_head[-1].weight)
        nn.init.zeros_(self.residual_head[-1].bias)

    def forward(self, x_img: torch.Tensor, x_tissue: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        h = self.img_norm(self.img_proj(x_img))
        base = self.base_head(h)
        gamma, beta = self.cond(x_tissue).chunk(2, dim=-1)
        h_res = h * (1.0 + gamma) + beta
        delta = self.residual_head(h_res)
        pred = base + delta
        return pred, base, delta


def train_epoch(model, loader, optim, loss_fn, device: torch.device, residual_l2: float) -> tuple[float, float]:
    model.train()
    total = 0.0
    total_delta = 0.0
    n = 0
    for xb_img, xb_tissue, yb in loader:
        ...
        pred, _, delta = model(xb_img, xb_tissue)
        main_loss = loss_fn(pred, yb)
        delta_penalty = delta.pow(2).mean()
        loss = main_loss + residual_l2 * delta_penalty
        loss.backward()
        optim.step()
        total += float(main_loss.item()) * xb_img.shape[0]
        total_delta += float(delta_penalty.item()) * xb_img.shape[0]
        n += xb_img.shape[0]
    return total / max(n, 1), total_delta / max(n, 1)


@torch.no_grad()
def eval_epoch(model, loader, loss_fn, device: torch.device) -> tuple[float, float, np.ndarray, np.ndarray]:
    model.eval()
    total = 0.0
    total_delta = 0.0
    n = 0
    ...
    for xb_img, xb_tissue, yb in loader:
        ...
        total_delta += float(delta.pow(2).mean().item()) * xb_img.shape[0]
        n += xb_img.shape[0]
        preds.append(pred.cpu().numpy())
        trues.append(yb.cpu().numpy())
    return total / max(n, 1), total_delta / max(n, 1), np.concatenate(preds, axis=0), np.concatenate(trues, axis=0)
...
def main() -> None:
    ...
    parser.add_argument("--tissue-cache", default=str(DEFAULT_TISSUE_CACHE))
    parser.add_argument("--fusion-tag", default="tissue4_residual")
    ...
    tissue_cache = Path(args.tissue_cache)
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_delta = train_epoch(model, train_loader, optim, loss_fn, device, args.residual_l2)
        val_loss, val_delta, _, _ = eval_epoch(model, val_loader, loss_fn, device)
        ...
        train_history.append(
            {
                ...
                "train_delta_l2": train_delta,
                "val_loss": val_loss,
                "val_delta_l2": val_delta,
            }
        )
        print(
            f"Epoch {epoch}: train_loss={train_loss:.4f} train_delta={train_delta:.6f} "
            f"val_loss={val_loss:.4f} val_delta={val_delta:.6f}",
            flush=True,
        )
    ...
    val_loss, val_delta, val_pred, val_true = eval_epoch(model, val_loader, loss_fn, device)
    test_loss, test_delta, test_pred, test_true = eval_epoch(model, test_loader, loss_fn, device)
    print(f"Final: val_loss={val_loss:.4f} test_loss={test_loss:.4f} val_delta={val_delta:.6f} test_delta={test_delta:.6f}", flush=True)
