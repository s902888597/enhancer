#!/usr/bin/env python3
"""Summarize eRNA_count>=4 z-score union top3000 fixed-split runs."""
...
def main() -> None:
    ...
    for cancer in CANCERS:
        ...
        for head in ["pan", "specific", "fused"]:
            rows.append({"cancer": cancer, "method": f"mixed_{head}", "pearson_mean": load_mixed(cancer, head)})
    out_df = pd.DataFrame(rows)
    out_csv = RUN_ROOT / "zscore_erna_ge4_top3000_fixedsplit_test_mean_pearson_summary.csv"
    ...
    ax.set_title("Fixed Split Top3000 z-score SE Prediction, eRNA_count >= 4 (Attention, k=300, mixup)", fontsize=13, weight="bold")
    ...
    fig.tight_layout()
    out_png = RUN_ROOT / "zscore_erna_ge4_top3000_fixedsplit_attention_comparison.png"
    fig.savefig(out_png)
    print(out_csv)
    print(out_png)
    print(pivot.round(4))
