...
#SBATCH --output=/taiga/illinois/vetmed/cb/kwang222/enhancer/MIL/average_embed/zscore_union21477_erna_ge4_top3000_fixedsplit/logs/mixed_singlehead_%j.out
#SBATCH --error=/taiga/illinois/vetmed/cb/kwang222/enhancer/MIL/average_embed/zscore_union21477_erna_ge4_top3000_fixedsplit/logs/mixed_singlehead_%j.err
...
"${PY}" "${ROOT}/MIL/average_embed/run_dualhead_three_cancer_attention_masked.py" \
  ...
  --out-dir "${RUN_ROOT}/mixed_singlehead_attention_k300_mixup" \
  --epochs 50 \
  --batch-size 4 \
  --num-workers 4 \
  --max-patches 300 \
