...
OUT_DIR = ROOT / "top1000_training_option_distribution"


def load_independent() -> pd.DataFrame:
    paths = {
        "BRCA": ROOT / "brca_top1000_mean_mlp_nopca_seed44" / "per_enhancer_correlation_test.csv",
        "LUAD": ROOT / "luad_top1000_mean_mlp_nopca_seed44" / "per_enhancer_correlation_test.csv",
        "SKCM": ROOT / "skcm_top1000_mean_mlp_nopca_seed44" / "per_enhancer_correlation_test.csv",
    }
    rows = []
    for cancer, path in paths.items():
...
def load_joint(subdir: str, option: str) -> pd.DataFrame:
    ...
    for cancer in ["BRCA", "LUAD", "SKCM"]:
        path = ROOT / "mix3_shared_specific_ablation" / subdir / f"per_enhancer_correlation_test_{cancer}.csv"
        tmp = pd.read_csv(path).copy()
        tmp["cancer"] = cancer
        tmp["option"] = option
        rows.append(tmp)
...
def make_plot(df: pd.DataFrame, out_path: Path) -> None:
    option_order = ["Independent", "Joint Single-Head", "Joint Shared+Specific"]
    cancer_order = ["BRCA", "LUAD", "SKCM"]
    colors = {
        "Independent": "#8FB8DE",
        "Joint Single-Head": "#C8C8C8",
        "Joint Shared+Specific": "#F2B880",
    }
    ...
    for ax, cancer in zip(axes, cancer_order):
        ...
        ax.set_xticks([1, 2, 3])
        ax.set_xticklabels(["Independent", "Single-head", "Shared+Specific"], rotation=18, ha="right")
        ax.grid(axis="y", alpha=0.2)
        ax.set_ylim(-0.35, 0.85)

    axes[0].set_ylabel("Per-enhancer test Pearson r")
    fig.suptitle("Top1000 Test Accuracy Distribution Across Training Strategies", fontsize=13, y=0.98)
    fig.tight_layout()
...
def main() -> None:
    ...
    df = pd.concat(
        [
            load_independent(),
            load_joint("shared_only", "Joint Single-Head"),
            load_joint("shared_plus_specific", "Joint Shared+Specific"),
        ],
        ignore_index=True,
    )
    ...
    make_plot(df, OUT_DIR / "top1000_training_option_violin.png")
