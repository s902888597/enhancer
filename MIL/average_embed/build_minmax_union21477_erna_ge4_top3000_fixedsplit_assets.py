#!/usr/bin/env python3
"""Build eRNA_count>=4 top3000 min-max fixed-split assets."""
...
def input_csv(cancer: str, split: str) -> Path:
    ...
    matches = sorted(IN_ROOT.glob(f"{cancer}_*{label}_avg_eRNA_minmax_per_SE.csv"))
    if len(matches) != 1:
...
def main() -> None:
    ...
    for cancer in CANCERS:
        for split in SPLIT_TO_LABEL:
            ...
            out_csv = label_dir / f"{cancer}_{split}_minmax_top3000_shared.csv"
    ...
    config = {
        ...
        "selection": "filter_eRNA_count_ge4_then_top3000_by_variance_across_BRCA_LUAD_SKCM_train_minmax_only",
        "splits": "fixed train/validation/test from uploaded minmax files",
